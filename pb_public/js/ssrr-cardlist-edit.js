...
class SsrrCardList extends SsrrList {
  ...
  set setSpellslinger(spellslingerStr) {
    const spellslinger = JSON.parse(spellslingerStr)
    switch(spellslinger.name) {
      case 'Drizzt':
        this.defaultFilter = `
          ((colour.name !~ 'Blue' && colour.name !~ 'Black' && colour.name !~ 'Red')
          || (legendary = true && type.name = 'Creature'))
          && ${this.#formatSignatureFilter(spellslinger)}`
        this.#resetSpellslingerColours()
        break
      case 'Davriel':
        this.defaultFilter = `
          ((colour.name = 'Black' || colour.name = 'Colourless')
          || (type.name = 'Spell'))
          && ${this.#formatSignatureFilter(spellslinger)}`
        this.#resetSpellslingerColours()
        break
      case 'Vivian':
        this.defaultFilter = `
          ((colour.name = 'Green' || colour.name = 'Colourless')
          || (type.name = 'Creature'))
          && ${this.#formatSignatureFilter(spellslinger)}`
        this.#resetSpellslingerColours()
        break
      case 'Nissa':
        this.defaultFilter = this.#formatSignatureFilter(spellslinger)
        this.#resetSpellslingerColours()
        break
      default:
        this.defaultFilter = this.#formatSignatureFilter(spellslinger)
        this.#setSpellslingerColours(spellslinger)
        break
    }
  }
  ...
  #formatSignatureFilter(spellslinger) {
    return `(set.name != 'Signatures' || ${spellslinger.signatures.map((sig) => `id = "${sig}"`).join('||')})`
  }

  #setSpellslingerColours(spellslinger) {
    const colourNames = spellslinger.expand.colour.map((c) => c.name.toLowerCase())
    colourNames.push('colourless')
    this.#colourFilter.querySelectorAll('input[type="checkbox"]').forEach((check) => {
      check.checked = colourNames.includes(check.id)
    })
    this.#colourFilter.querySelector(`option[value="${spellslinger.name == 'Serra' ? 'include' : 'at-most'}"]`).selected = true
  }

  #resetSpellslingerColours() {
    this.#colourFilter.querySelectorAll('input[type="checkbox"]').forEach((check) => {
      check.checked = false
    })
    this.#colourFilter.querySelector(`option[value="at-most"]`).selected = true
  }
